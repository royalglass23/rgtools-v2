...
export default async function CalculatorPricingAdminPage({
  ...
  const selectedVersionId = Array.isArray(params.version)
    ? params.version[0]
    : params.version;
    ...
    <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            ...
            <span className="font-mono text-gray-700">
              {activeVersion?.versionLabel ?? "None seeded"}
            </span>
          </p>
        </div>
        {readOnly && (
          <Link
            ...
            className="rounded border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 shadow-sm"
          >
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-3">
                      {row.isActive ? (
                        ...
                        <Link
                          href={`/admin/calculator-pricing?version=${row.id}`}
                          className="text-sm text-blue-700 hover:text-blue-900"
                        >
                          View
                        </Link>
                      ...
                      {!row.isActive && (
                        <>
                          <details className="relative">
                            ...
                            <form
                              action={activatePricingConfigVersionAction}
                              className="absolute right-0 z-10 mt-2 w-72 rounded border border-gray-200 bg-white p-3 shadow-lg"
                            >
                              <input
                                type="hidden"
                                name="versionId"
                              ...
                              <button
                                type="submit"
                                className="mt-2 rounded bg-green-700 px-3 py-1.5 text-sm text-white"
                              >
                                Activate
                              </button>
                            </form>
                          ...
                          <details className="relative">
                            ...
                            <form
                              action={deletePricingConfigVersionAction}
                              className="absolute right-0 z-10 mt-2 w-72 rounded border border-gray-200 bg-white p-3 shadow-lg"
                            >
                              <input
                                type="hidden"
                              ...
                              <button
                                type="submit"
                                className="mt-2 rounded bg-red-700 px-3 py-1.5 text-sm text-white"
                              >
                                Delete
                              </button>
                            </form>
      ...
      {selectedVersion ? (
        ...
        <div className="rounded border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          Seed the first pricing version before editing calculator prices in
          rgtools.
        </div>
      )}
    </div>
...
function formatDate(value: Date) {
  return `${value.toLocaleDateString("en-NZ")} ${value.toLocaleTimeString(
    "en-NZ",
    {
      hour: "2-digit",
      minute: "2-digit",
