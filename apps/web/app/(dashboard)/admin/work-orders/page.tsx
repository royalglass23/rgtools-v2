...
export default async function WorkOrderConfigurationPage() {
  ...
  const { installers, stages, hardwareStatuses } = await getWorkOrderConfigLists()
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <ConfigPanel title="Installers" action={createWorkOrderInstallerAction} rows={installers} />
        <ConfigPanel title="Stages" action={createWorkOrderStageAction} rows={stages} />
        <ConfigPanel title="Hardware Statuses" action={createWorkOrderHardwareStatusAction} rows={hardwareStatuses} />
      </div>
    </div>
  )
}
