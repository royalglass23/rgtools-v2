...
export function DashboardShell({
  ...
  const [openGroup, setOpenGroup] = useState<string | null>(() =>
    navigation.find((entry) => entry.kind === 'group' && groupIsActive(entry, pathname))?.id ??
    null,
  )
  ...
  function toggleGroup(entry: Extract<DashboardNavigationEntry, { kind: 'group' }>) {
    if (collapsed) {
      setCollapsed(false)
    ...
    <div className={styles.shell} data-collapsed={collapsed || undefined}>
      ...
      <aside className={styles.sidebar} data-mobile-open={mobileOpen || undefined}>
        ...
        <div className={styles.navigationEntries}>
          {navigation.map((entry) => {
            ...
            return (
              <div key={entry.id} className={styles.navigationGroup} data-active={active || undefined}>
                <button
                  type="button"
                  className={styles.navigationLink}
                ...
                >
                  ...
                  <span className={styles.navigationText}>{entry.label}</span>
                  <span className={styles.chevron} data-expanded={expanded || undefined} aria-hidden="true">
                    <Icon name="chevron" />
                  </span>
                </button>
                <div
      ...
      <div className={styles.pageColumn}>
        <header className={styles.mobileHeader}>
          ...
          <Link href="/" className={styles.mobileBrand} aria-label="Royal Glass tools home">
...
function Icon({ name }: { name: IconName }) {
  const paths: Record<IconName, ReactNode> = {
    ...
    'lead-intake': <><path d="M12 3v18M3 12h18"/><circle cx="12" cy="12" r="9"/></>,
    'quote-tracker': <><path d="M5 3h14v18l-3-2-4 2-4-2-3 2V3Z"/><path d="M9 8h6M9 12h6"/></>,
    'work-order': <><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M8 3v4M16 3v4M8 11h8M8 15h5"/></>,
    'ps-generator': <><path d="M6 3h9l4 4v14H6V3Z"/><path d="M14 3v5h5M9 13h6M9 17h4"/></>,
    clients: <><circle cx="9" cy="8" r="3"/><path d="M3 20c0-4 2-7 6-7s6 3 6 7M16 5c3 0 5 2 5 5s-2 5-5 5"/></>,
    ...
    module: <><path d="m12 3 8 4.5-8 4.5-8-4.5L12 3Z"/><path d="m4 12 8 4.5 8-4.5M4 16.5l8 4.5 8-4.5"/></>,
