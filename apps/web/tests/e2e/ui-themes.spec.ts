...
const screenshotDirectory = path.resolve(process.cwd(), 'output/playwright/mt-200')
...
async function findWorkOrderDetailPath(page: Page) {
  ...
  const detailLink = page.locator('a[href^="/work-orders/"]').first()
...
test.describe('Royal Glass Precision themes', () => {
  ...
  test('captures matching Light and Dark reference screens', async ({ page }) => {
    const workOrderPath = await findWorkOrderDetailPath(page)
    test.skip(!workOrderPath, 'MT-200 Work Order detail screenshot requires at least one visible work order.')
    ...
    for (const theme of ['Light', 'Dark'] as const) {
      for (const screen of screens) {
        ...
        await page.screenshot({
          path: path.join(screenshotDirectory, `${screen.name}-${theme.toLowerCase()}.png`),
          fullPage: true,
        })
      }
    }
  ...
  test('keeps the mobile shell navigable without viewport overflow', async ({ page }) => {
  ...
  test('shows keyboard focus and honours reduced motion', async ({ page }) => {
    ...
    const transitionDuration = await page.locator('aside').evaluate(
